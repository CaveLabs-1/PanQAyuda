...
class TestEliminarPaquete(TestCase):
    ...
    def test_ac_23_1(self):
        ...
        self.assertEqual(resp.context['catalogo_paquetes'][0], 1)

        paquete_inventario = PaqueteInventario.objects.first()
        #borrar paquete
        self.client.get(reverse('paquetes:borrar_paquete_inventario', kwargs={'id_paquete':paquete_inventario.id}))

        #Verificar cantidad
        resp = self.client.get(reverse('paquetes:lista_paquete_inventario'))
        self.assertEqual(resp.context['catalogo_paquetes'][0], 0)
